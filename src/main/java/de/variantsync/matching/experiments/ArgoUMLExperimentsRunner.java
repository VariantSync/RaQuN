...
public abstract class ArgoUMLExperimentsRunner extends AbstractRQRunner {
    private String subsetID = null;
    private static final int FIRST_SUBSET = 1;
    private static final int LAST_SUBSET = 30;

    public ArgoUMLExperimentsRunner(final String... args) {
        super(args);
        if (args.length > 1) {
    ...
    protected abstract List<String> matcherList();
    ...
    public void run() {
        ...
        if (subsetID == null) {
            timeoutSubset = "p" + largestSubset() + "_s030";
        } else {
            timeoutSubset = "p" + largestSubset() + "_" + subsetID;
        }
